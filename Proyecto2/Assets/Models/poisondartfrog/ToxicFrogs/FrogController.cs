...
public class FrogController : MonoBehaviour
{
    ...
    public void RotateLeft(float angle)
    {
        RootMotion();
        DestroyGuts();
        frog.transform.Rotate(0, -angle, 0); 
    }
    ...
    public void TurnLeftWithRotation(float angle)
    {
        TurnLeft(); // Activa la animación de girar a la izquierda
        RotateLeft(angle); // Ajusta la rotación manualmente si es necesario
    }

    public void TurnRightWithRotation(float angle)
    {
        TurnRight(); // Activa la animación de girar a la derecha
        RotateRight(angle); // Ajusta la rotación manualmente si es necesario
    }
